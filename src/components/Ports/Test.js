...
const useUploadAndDisplayImageStyles = makeStyles((theme) => ({
    uploadAndDisplayImage: {},
}))
export const UploadAndDisplayImage = () => {
    const classes = useUploadAndDisplayImageStyles();
    const [selectedImage, setSelectedImage] = useState(null);

    const handleSelectImage = async (event) => {
        setSelectedImage(await convertToBase64(event.target.files[0]))
    }
