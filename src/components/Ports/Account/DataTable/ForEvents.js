...
const headCells = [
  ...
  { id: 'disctiption', numeric: false, disablePadding: false, label: 'Disctiption' },
];
...
export const EnhancedTable = (props) => {
  const rows = props.state.map(row => ({
    id: row.id,
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell component="th" id={labelId} scope="row" padding="none"
                        className={`profile__events__type__error ${row.typeError.toLowerCase()}`}>
                        {row.typeError.toLowerCase() === 'critical' ? 'Critical:' : ''}
                        {row.typeError.toLowerCase() === 'warning' ? 'Warning:' : ''}
                        {row.typeError.toLowerCase() === 'regular' ? 'Regular:' : ''}
                      </TableCell>
                      ...
                      <TableCell align="left">{row.disctiption}</TableCell>
                    </TableRow>
                  );
                })}
              {emptyRows > 0 && (
